...
import (
	...
	server "microless/hotelreservation/user/internal/userserver"
...
const (
	appName  = "User"     // name of the application shown in jaeger
	colName  = "user"     // name of mongodb collection
	colIndex = "username" // new index for mongodb collection
)
...
func main() {
	...
	// create index
	err = utils.CreateIndex(ctx, col, colIndex, true)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	mc := otelmemcache.NewClientWithTracing(memcache.New(config.Memcached.User))
