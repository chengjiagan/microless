...
import (
	...
	server "microless/media/reviewstorage/internal/reviewstorageserver"
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "ReviewStorage", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	rdb, err := utils.NewRedisClient(config.Redis.ReviewStorage)
	if err != nil {
		logger.Fatal(err.Error())
	}
	defer func() {
	...
	col := mongodb.Database(config.MongoDB.Database).Collection("review")
	...
	pb.RegisterReviewStorageServiceServer(grpcServer, server)
