...
import (
	...
	server "microless/media/userreview/internal/userreviewserver"
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "UserReview", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	rdb, err := utils.NewRedisClient(config.Redis.UserReview)
	if err != nil {
		logger.Fatal(err.Error())
	}
	defer func() {
	...
	col := mongodb.Database(config.MongoDB.Database).Collection("user-review")
	...
	pb.RegisterUserReviewServiceServer(grpcServer, server)
