...
import (
	...
	server "microless/media/castinfo/internal/castinfoserver"
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "CastInfo", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	rdb, err := utils.NewRedisClient(config.Redis.CastInfo)
	if err != nil {
		logger.Fatal(err.Error())
	}
	defer func() {
	...
	col := mongodb.Database(config.MongoDB.Database).Collection("cast-info")
	...
	pb.RegisterCastInfoServiceServer(grpcServer, server)
