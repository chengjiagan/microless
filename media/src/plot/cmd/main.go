...
import (
	...
	server "microless/media/plot/internal/plotserver"
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "Plot", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	rdb, err := utils.NewRedisClient(config.Redis.Plot)
	if err != nil {
		logger.Fatal(err.Error())
	}
	defer func() {
	...
	col := mongodb.Database(config.MongoDB.Database).Collection("plot")
	...
	pb.RegisterPlotServiceServer(grpcServer, server)
