...
import (
	...
	server "microless/media/moviereview/internal/moviereviewserver"
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "MovieReview", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	rdb, err := utils.NewRedisClient(config.Redis.MovieReview)
	if err != nil {
		logger.Fatal(err.Error())
	}
	defer func() {
	...
	col := mongodb.Database(config.MongoDB.Database).Collection("movie-review")
	if err := utils.CreateIndex(ctx, col, "movie_id"); err != nil {
		logger.Fatal(err.Error())
	}
	...
	pb.RegisterMovieReviewServiceServer(grpcServer, server)
