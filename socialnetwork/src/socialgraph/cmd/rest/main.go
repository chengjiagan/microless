...
import (
	...
	gw "microless/socialnetwork/proto/socialgraph"
)
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "SocialGraphRest", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	err = gw.RegisterSocialGraphServiceHandler(ctx, mux, conn)
	if err != nil {
		logger.Fatal(err.Error())
	}
