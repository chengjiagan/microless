...
import (
	...
	gw "microless/socialnetwork/proto/composepost"
)
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "ComposePostRest", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	err = gw.RegisterComposePostServiceHandler(ctx, mux, conn)
	if err != nil {
		logger.Fatal(err.Error())
	}
