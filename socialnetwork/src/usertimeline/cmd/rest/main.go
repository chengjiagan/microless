...
import (
	...
	gw "microless/socialnetwork/proto/usertimeline"
)
...
func main() {
	...
	tp, err := utils.NewTracerProvider(ctx, "UserTimelineRest", config.Otel)
	if err != nil {
		logger.Fatal(err.Error())
	}
	...
	err = gw.RegisterUserTimelineServiceHandler(ctx, mux, conn)
	if err != nil {
		logger.Fatal(err.Error())
	}
